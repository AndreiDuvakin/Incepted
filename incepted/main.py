...
def admin_user(login_usr):
    if current_user.is_authenticated:
        ...
        if current_user.role == 1:
            with db_session.create_session() as data_session:
        ...
        if current_user.role == 1:
            with db_session.create_session() as data_session:
                ...
                if request.method == 'POST':
                    ...
                    list(
                        map(lambda x: save_admin_data(x, data_session),
                            list(filter(lambda x: 'role_' in x[0], data_form))))
                    activ_id = list(
                        map(lambda user: int(user[0].split('_')[-1]),
                            list(filter(lambda x: 'active_' in x[0], data_form))))
                    banned_id = list(
                        map(lambda user: int(user[0].split('_')[-1]),
                            list(filter(lambda x: 'banned_' in x[0], data_form))))
                    for user in users:
                        if int(user.role) != 1:
        ...
        with db_session.create_session() as data_session:
            ...
            if current_project:
                ...
                if current_user.id == current_project.creator or current_user.role == 1:
                            ...
                            for i in staff:
                                if i not in new_staff:
                                    data_session.delete(data_session.query(StaffProjects).filter(
                                        StaffProjects.user == i['id'],
                                        StaffProjects.project == current_project.id).first())
                            data_session.commit()
                        if form.logo.data:
                    ...
                    return render_template('edit_project.html', title='Изменение проекта', form=form,
                ...
                if current_user.id == current_project.creator or current_user.id in list(
                    ...
                    if form_file.validate_on_submit():
                        if form_file.file.data[0].filename:
                            files = list(
                                map(lambda x: save_proof_quest(current_project, x, current_user.id),
                                    form_file.file.data))
                        return redirect(f'/project/{str(current_project.id)}')
